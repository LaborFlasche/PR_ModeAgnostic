...
def toy_rf_data():
    ...
    X = pd.DataFrame(rng.random((120, 3)), columns=["f0", "f1", "f2"])
...
def test_csv_created_after_run(runner):
    ...
    bench.run(model, X, run_meta={"dataset": "test_ds", "model": "rf", "n_features": 3, "n_samples": 120})
...
def test_csv_appends_on_second_run(runner):
    bench, model, X = runner
    meta = {"dataset": "test_ds", "model": "rf", "n_features": 3, "n_samples": 120}
...
def test_n_eval_limits_explained_samples(tmp_path, toy_rf_data):
    ...
    bench = BenchmarkRunner(
        backends=[ShapTrueValueBackend],
        output_csv=str(tmp_path / "results.csv"),
        n_background=100,
        n_eval=10,
    )
    bench.run(model, X, run_meta={"dataset": "ds", "model": "rf", "n_features": 3, "n_samples": 120})
    df = pd.read_csv(bench.output_csv)
...
def test_raises_when_x_too_small(tmp_path, toy_rf_data):
    ...
    with pytest.raises(ValueError, match="no evaluation rows remain"):
        bench.run(model, X_small, run_meta={"dataset": "ds", "model": "rf", "n_features": 3, "n_samples": 50})
